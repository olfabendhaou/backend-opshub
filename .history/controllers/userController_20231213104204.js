...
const {createUserActiondb} =require ("/Users/olfabendhaou/Desktop/Internship-Backend/db/userDb")
...
const createUserAction = async (req, res) => {
  ...
  try{
    const action_user = await createUserActiondb({login,name, surname, email, action, action_time});
    console.log (action_user);
    res.status(200).json({
    status: "action-user added successfully",
    action_user,
  })
  }
  catch(err){
    console.error('Error creating user action:', err);
    res.status(400).send({msg : "can not add action_user"}) 
  }
  };
...
const updateUser = async (req, res) => {
    const {firstname, lastname, jobtitle, company, city, postalcode,country, state, language, timezone, telephone, emailadress} = req.body;
  ...
  const changeUserPassword = async (req, res) => {
    ...
    try {
      if (req.user.id !== id){
        return res.status(400).json({ errors: [{ msg: "you are not authorized to change password! " }] });
      }
      console.log('hello');
      ...
      { 
        ...
        console.log(user.rows[0])
        return res.status(200).json({success : [{ message: `Password updated successfully! Your new password is : ${newpassword}` ,user.rows[0]}]});
    } }
    catch (error) {
