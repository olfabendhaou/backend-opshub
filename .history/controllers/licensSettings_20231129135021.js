...
const uploadfile =async(req,res) => {
    let cloudinary_id, url;
    try {
         // Upload file to cloudinary
        const result = await cloudinary.uploader.upload(req.file.path,{ resource_type: 'raw' });
        const url=result.secure_url
        const cloudinary_id=result.public_id
        const fileContent = await fs.promises.readFile(url, 'utf-8');

        res.status(200).send({msg : "File uploded successfully ",cloudinary_id,url,filecontent})
    }
    catch (Error) { 
