...
router.post("/login",loginValidator(),validator,async(req, res) => {
    ...
    try {
      ...
      const existingToken = Object.keys(tokens).find(
        (token) => tokens[token].userId === userId && jwt.verify(token, process.env.SCRT_KEY, {ignoreExpiration:true })

      );
      if (existingToken) {
        const decoded = jwt.verify(existingToken, process.env.SCRT_KEY, { ignoreExpiration: true });
        const isTokenExpired = decoded.exp * 1000 < Date.now();
        if(!isTokenExpired){
         returnres.json({msg: 'token already exist', token: existingToken ,user:founduser.rows[0]});
        }

      }
      const token = jwt.sign({id:userId},process.env.SCRT_KEY,{expiresIn : "10s"});
