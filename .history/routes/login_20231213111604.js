...
router.post("/login",loginValidator(),validator,async(req, res) => {
    ...
    try {
      ...
      const existingToken = Object.keys(tokens).find(
        (token) => tokens[token].userId === userId && jwt.verify(token, secretKey, { ignoreExpiration: true })
      );
      const token = jwt.sign({id:founduser.rows[0].id},process.env.SCRT_KEY,{expiresIn : "1h"});
