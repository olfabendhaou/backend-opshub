...
router.post("/login",loginValidator(),validator,async(req, res) => {
    ...
    try {
      ...
      if (existingToken) {
        return res.json({msg: 'token already exist', token: existingToken });
      }
      const token = jwt.sign({id:userId},process.env.SCRT_KEY,{expiresIn : "1h"});
      okens[token] = { userId, expirationTime: jwt.decode(token).exp };
