...
/////////////////Get user By id////////////////////
const getUserByIdDb = async function(req.user.id) {
    try {
      const query = {
...
const updateUserDb = async ({
  firstname, lastname, jobtitle, company, city, postalcode,country, state, language, timezone, telephone, emailadress
},id) => {
  const { rows: Users2 } = await pool.query(
    `UPDATE Users2
    SET firstname=$1, lastname=$2, jobtitle=$3, company=$4, city=$5, postalcode=$6, country=$7, state=$8, language=$9, timezone=$10, telephone=$11, emailadress=$12
    WHERE id = $13
    RETURNING firstname, lastname, jobtitle, company, city, postalcode, country, state, language, timezone, telephone, emailadress, id
    `,
    [firstname, lastname, jobtitle, company, city, postalcode,country, state, language, timezone, telephone, emailadress,id]
  );
  return Users2[0];
};
///////////////delete user/////////////
